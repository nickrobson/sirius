...
public class ProgressCommand extends Command {
    ...
    public void exec(TelepadBot bot, Message message, String[] args) {
        ...
        if (JavaOMDB.IMDB_ID_PATTERN.matcher(args[0]).matches()) {
            series = OmniBot.getSeriesController().getSeries(args[0], false);
        } else {
            series = OmniBot.getSeriesController().getSeriesByLink(args[0]);
        }
        if (series == null) {
        ...
        String m;
        String progress = OmniBot.getProgressController().getProgress(message.getSender(), series.getId());
        if (args.length == 1) {
            if (progress == null) {
            ...
            if (EPISODE_PATTERN.matcher(args[1]).matches()) {
                OmniBot.getProgressController().setProgress(message.getSender(), series.getId(), args[1]);
